...
clamp();
module clamp(dia=6,n=3,base=3,depth=10,hole=4,
lugs=true){
    height=dia/2+base;
    width=dia+4;
        for(i=[0:n-1]){
...
module lug(width=10,t=3,height=10,hole=4,socket=1){
    difference(){
        union(){
            ...
            cylinder(h=height,d=hole+2+socket*2,$fn=20);
        }
        translate([0,0,-1]) cylinder(h=height+2,d=hole,$fn=20);
        if(socket) translate([0,0,t+.1]) cylinder(h=height-t+0.1,d=hole+socket*2,$fn=20);
    }
}
